...
import {
  Droplet, Search, Plus, Edit, Trash2, RefreshCw,
  Calendar, Download, Eye, CheckCircle2, AlertCircle,
  FileText, Home, Users, TrendingUp, BarChart3, Clock, DollarSign
} from 'lucide-react';
import { utilityAPI, apartmentAPI } from '../api';
import { Card, Button, Input, Badge, Modal, StatCard } from '../components/UI';
import { formatDate, money } from '../utils/formatters';

export default function WaterManagement({ flash }) {
  ...
  const fetchReadings = useCallback(async () => {
    try {
      ...
      console.log('📊 Water readings:', res);
      ...
      if (flash) flash('❌ ' + (error.response?.data?.message || 'Không thể tải dữ liệu nước'));
      setReadings([]);
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-5">
      ...
      <Card className="p-5">
        <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
          <div>
            <h3 className="text-base font-bold text-slate-950">Quản lý nước</h3>
            <p className="text-sm text-slate-500">
              Quản lý chỉ số nước tiêu thụ của các căn hộ.
              <span className="ml-2 text-[#1f4f46] font-semibold">
                {stats.totalConsumption.toFixed(0)} m³ tổng tiêu thụ
              </span>
            </p>
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <StatCard icon={Droplet} label="Tổng tiêu thụ" value={`${stats.totalConsumption.toFixed(0)} m³`} hint="Tất cả căn hộ" />
        <StatCard icon={TrendingUp} label="Trung bình" value={`${stats.avgConsumption.toFixed(1)} m³`} hint="Mỗi lần nhập" />
        <StatCard icon={BarChart3} label="Cao nhất" value={`${stats.maxConsumption.toFixed(0)} m³`} hint="Một lần nhập" />
      </div>
            ...
            <Card key={reading.ReadingID} className="group hover:border-[#1f4f46]/30 transition-all">
              <div className="p-5">
                <div className="flex items-start justify-between">
                  ...
                  <Badge tone="green">
                    {reading.Consumption ? `${reading.Consumption.toFixed(0)} m³` : '0 m³'}
                  </Badge>
                </div>
                ...
                <div className="mt-4 space-y-2 text-sm">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-bold text-slate-950">{reading.OldIndex?.toFixed(1)} m³</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-slate-500">Chỉ số mới</span>
                    <span className="font-bold text-slate-950">{reading.NewIndex?.toFixed(1)} m³</span>
                  </div>
                  <div className="flex items-center justify-between">
      ...
      {/* Modal - giống ElectricityManagement */}
      <Modal
        open={modalOpen}
        title={modalMode === 'create' ? 'Nhập chỉ số nước' : 'Chi tiết chỉ số nước'}
        description={modalMode === 'create' ? 'Nhập chỉ số nước tiêu thụ cho căn hộ' : 'Xem chi tiết chỉ số nước'}
        ...
        {modalMode === 'view' && selectedReading ? (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Badge tone="green">
                {selectedReading.Consumption?.toFixed(0)} m³
              </Badge>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="rounded-xl border border-slate-200 p-4">
                ...
                <div className="mt-2 space-y-2 text-sm">
                  <div><span className="text-slate-500">Chỉ số cũ:</span> {selectedReading.OldIndex?.toFixed(1)} m³</div>
                  <div><span className="text-slate-500">Chỉ số mới:</span> {selectedReading.NewIndex?.toFixed(1)} m³</div>
                  <div><span className="text-slate-500">Tiêu thụ:</span> <span className="font-bold text-[#1f4f46]">{selectedReading.Consumption?.toFixed(1)} m³</span></div>
                </div>
              </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Form giống ElectricityManagement nhưng utilityTypeId = 2 */}
            <div>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div>
                <label className="mb-1 block text-sm font-semibold text-slate-700">Chỉ số cũ (m³)</label>
                <Input
                  type="number"
                  value={form.oldIndex}
              ...
              <div>
                <label className="mb-1 block text-sm font-semibold text-slate-700">Chỉ số mới (m³) *</label>
                <Input
                  type="number"
                  value={form.newIndex}

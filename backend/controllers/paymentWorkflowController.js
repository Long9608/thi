...
async function readInvoice(pool, id, lock = false) {
    const result = await pool.request().input('ID', sql.Int, id).input('PaidStatus', sql.Int, SUCCESS_PAYMENT_STATUS_ID).query(`
        SELECT i.*, c.ContractNumber, a.ApartmentCode,
          CASE WHEN i.DueDate<CAST(GETDATE() AS date) THEN 1 ELSE 0 END IsOverdue,
          i.TotalAmount-ISNULL((SELECT SUM(Amount) FROM Payment WHERE InvoiceID=i.InvoiceID AND StatusID=@PaidStatus),0) RemainingAmount,
                ...
                OUTER APPLY (SELECT TOP 1 s.SubmittedAt,s.ConfirmedAt,s.SubmittedBy,s.TransferContent
                                         FROM InvoicePaymentSubmission s WHERE s.InvoiceID=i.InvoiceID AND s.ConfirmedAt IS NULL
                                         ORDER BY s.SubmittedAt DESC) s
...
function payable(invoice) {
    ...
    if (invoice.RemainingAmount <= 0) fail(409, 'Hóa đơn đã được xác nhận thanh toán');
}
exports.getPaymentInfo = async (req, res, next) => {
    try {
        ...
        const qrUrl = isResidentAccount(req) && config && !invoice.IsOverdue && invoice.RemainingAmount > 0 && invoice.StatusID !== 4 && invoice.WorkflowStatus !== 'DRAFT'
...
exports.submitPayment = async (req, res, next) => {
    try {
        ...
        await inTransaction(pool, async tx => {
            ...
            if (invoice.IsOverdue) fail(409, 'Hóa đơn quá hạn; vui lòng xin gia hạn thanh toán');
            ...
            await notifyUsers(tx, { senderId: req.userId, title: `Kiểm tra thanh toán hóa đơn #${id}`,
                content: `Cư dân căn ${invoice.ApartmentCode} báo đã chuyển ${invoice.RemainingAmount.toLocaleString('vi-VN')} đ. Vào Hóa đơn & thu phí để kiểm tra và xác nhận hóa đơn #${id}.`, userIds: recipients.recordset.map(r => r.UserID), entityType: 'Invoice', entityId: id, targetPage: 'fees' });
        });
...
exports.confirmPayment = async (req, res, next) => {
    try {
        ...
        await inTransaction(pool, async tx => {
            ...
            await notifyUsers(tx, { senderId: req.userId, title: `Hóa đơn #${id} đã thanh toán`, content: `Ban quản lý/Kế toán đã xác nhận thanh toán hóa đơn #${id}, căn ${invoice.ApartmentCode}.`, userIds: users.recordset.map(r => r.UserID), entityType: 'Invoice', entityId: id, targetPage: 'fees' });
        });

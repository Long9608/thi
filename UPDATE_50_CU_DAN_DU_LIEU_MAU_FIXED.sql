/*
    ...
    Compatible with thi(20260923-150153) / ApartmentManagement.
...
BEGIN TRY
    ...
    IF @ResidentRoleID IS NULL OR @ActiveContractStatusID IS NULL OR @OccupiedRoomStatusID IS NULL
       ...
       OR @PaymentSuccessID IS NULL OR @BankMethodID IS NULL OR @ParkingAreaID IS NULL
...
(50, N'Hồ Thanh Hương', 0, '1998-08-03', '0930000050', 'cudan50@ducvutower.demo', 'resident50', N'210 Lý Thường Kiệt, Quận 10, TP.HCM', N'Hồ Thanh Bình', '0830000050', '079980000050');
    ...
    SELECT u.UserID,@ResidentRoleID,'2026-09-01',1
    ...
    SELECT v.VehicleID,c.ContractID,pc.CardID,c.StartDate,c.EndDate,vt.MonthlyFee,'ACTIVE',1,SYSDATETIME(),SYSDATETIME()
    FROM #People p
    JOIN dbo.Resident r ON r.Email=p.Email
    JOIN dbo.Vehicle v ON v.ResidentID=r.ResidentID
...
BEGIN CATCH
    ...
    RAISERROR(N'Lỗi tạo dữ liệu demo: %s',16,1,@Err);
END CATCH;
GO
